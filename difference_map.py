...
figure()
imshow(clip((plot_I_1-plot_I_2) * 10 **3, plot_w0_mask_1), origin='lower', interpolation='nearest', extent=[min_az, max_az, min_el, max_el], aspect='auto')
title('Ground template I difference, $\mu K_{CMB}$')
xlabel('Azimuth, $^\circ$')
ylabel('Elevation, $^\circ$')
colorbar()
clim(vmin=-2100, vmax=1850)
savefig('dI_gt_%s.png'%sys.argv[3])

figure()
imshow(clip((plot_Q_1-plot_Q_2) * 10**3, plot_w4_mask_1), origin='lower', interpolation='nearest', extent=[min_az, max_az, min_el, max_el], aspect='auto')
title('Ground template Q difference, $\mu K_{CMB}$')
xlabel('Azimuth, $^\circ$')
ylabel('Elevation, $^\circ$')
colorbar()
clim(vmin=-220, vmax=300)
savefig('dQ_gt_%s.png'%sys.argv[3])

figure()
imshow(clip((plot_U_1-plot_U_2) * 10**3, plot_w4_mask_1), origin='lower', interpolation='nearest', extent=[min_az, max_az, min_el, max_el], aspect='auto')
title('Ground template U difference, $\mu K_{CMB}$')
xlabel('Azimuth, $^\circ$')
ylabel('Elevation, $^\circ$')
colorbar()
clim(vmin=-280, vmax=440)
savefig('dU_gt_%s.png'%sys.argv[3])

figure()
imshow(clip((plot_mag_1-plot_mag_2)*10**3, plot_w4_mask_1), origin='lower', interpolation='nearest', extent=[min_az, max_az, min_el, max_el], aspect='auto')
title('Az-El polarized power $\sqrt{Q^2 + U^2}$ difference, $\mu K_{CMB}$')
xlabel('Azimuth, $^\circ$')
ylabel('Elevation, $^\circ$')
gca().grid(True)
colorbar()
clim(vmin=-410, vmax=560)
